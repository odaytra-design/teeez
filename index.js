...
async function listCustomers(db) {
  if (!db) {
    ...
    for(const o of DEMO_ORDERS){
      if(!map.has(o.customer_phone)) map.set(o.customer_phone,{name:o.customer_name,phone:o.customer_phone,governorate:o.governorate,orders:0,total:0,last_order:o.created_at});
      const c=map.get(o.customer_phone); c.orders++; c.total+=Number(o.total||0);
      if(new Date(o.created_at)>new Date(c.last_order)) c.last_order=o.created_at;
    }
    return [...map.values()].sort((a,b)=>b.orders-a.orders);
  }
  const r=await db.prepare(`SELECT customer_name AS name,customer_phone AS phone,governorate,COUNT(*) AS orders,
    COALESCE(SUM(total),0) AS total,MAX(created_at) AS last_order
    FROM orders GROUP BY customer_phone,customer_name,governorate ORDER BY last_order DESC`).all();
  return r.results||[];
}
...
async function customersPage(env){
 ...
 const tr=rows.map(c=>`<tr><td><strong>${esc(c.name)}</strong></td><td>${esc(c.phone)}</td><td>${esc(c.governorate)}</td><td>${c.orders}</td><td>${Number(c.total).toFixed(2)}</td><td>${new Date(c.last_order).toLocaleDateString("ar-JO")}</td></tr>`).join("");
 return htmlResponse(`<header class="top"><div class="wrap"><div class="brand">Syria Commerce</div><div class="sub">إدارة العملاء</div></div></header>
 <div class="layout"><nav><a href="/">🏠 الرئيسية</a><a href="/dashboard">👥 المسوقون</a><a href="/products">📦 المنتجات</a><a href="/orders">🧾 الطلبات</a><a href="/commissions">💰 العمولات</a><a href="/customers">👤 العملاء</a><a href="/reports">📊 التقارير</a><a href="/settings">⚙️ الإعدادات</a></nav>
 <main><div class="card section"><span class="badge">المرحلة 7</span><h1>نظام العملاء</h1><p class="muted">تجميع العملاء تلقائياً من الطلبات وعرض تاريخ الشراء الأساسي.</p></div>
 <div class="grid"><div class="card stat"><div class="muted">عدد العملاء</div><div class="num">${rows.length}</div></div><div class="card stat"><div class="muted">إجمالي المبيعات</div><div class="num">${total.toFixed(2)}</div></div></div>
 <div class="card section"><h2>قائمة العملاء</h2><div style="overflow:auto"><table><thead><tr><th>الاسم</th><th>الهاتف</th><th>المحافظة</th><th>الطلبات</th><th>إجمالي الشراء</th><th>آخر طلب</th></tr></thead><tbody>${tr||"<tr><td colspan=6>لا يوجد عملاء بعد</td></tr>"}</tbody></table></div></div>
 <div class="card section"><h2>طريقة العمل</h2><p class="muted">لا يحتاج المسوق لإضافة العميل بشكل منفصل؛ العميل يظهر تلقائياً عند تسجيل أول طلب له. عند ربط D1 لاحقاً سيصبح السجل دائماً.</p></div>
 </main></div>
 <style>
 *{box-sizing:border-box}body{margin:0;background:#f5f7fb;color:#172033;font-family:Arial,sans-serif}.top{background:#111827;color:#fff;padding:18px 22px}.wrap{max-width:1100px;margin:auto}.brand{font-size:24px;font-weight:700}.sub{opacity:.75;margin-top:5px}.layout{display:grid;grid-template-columns:220px 1fr;gap:18px;max-width:1100px;margin:22px auto;padding:0 16px}nav,.card{background:#fff;border:1px solid #e5e7eb;border-radius:16px;box-shadow:0 4px 18px #00000008}nav{padding:10px;height:max-content}nav a{display:block;padding:13px;border-radius:10px;color:#172033;text-decoration:none}nav a:hover{background:#f1f5f9}.section,.stat{padding:20px}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:14px;margin:14px 0}.num{font-size:28px;font-weight:700;margin-top:8px}.muted{color:#667085}.badge{display:inline-block;padding:6px 10px;border-radius:999px;background:#eef2ff}table{width:100%;border-collapse:collapse}th,td{padding:12px;border-bottom:1px solid #eee;text-align:right;white-space:nowrap}@media(max-width:700px){.layout{grid-template-columns:1fr}nav{display:grid;grid-template-columns:1fr 1fr}}
 </style>`,"العملاء");
}
...
async function reportsPage(env){
 ...
 const products=await listProducts(getStore(env));
 const customers=await listCustomers(getStore(env));
 const delivered=orders.filter(o=>o.status==="delivered");
 const cancelled=orders.filter(o=>o.status==="cancelled");
 const revenue=delivered.reduce((a,o)=>a+Number(o.total||0),0);
 const commissions=delivered.reduce((a,o)=>a+Number(o.commission||0),0);
 const avg=delivered.length?revenue/delivered.length:0;
 const byProduct={};
 for(const o of orders){const k=o.product_name||"غير معروف";if(!byProduct[k])byProduct[k]={name:k,orders:0,revenue:0};byProduct[k].orders++;byProduct[k].revenue+=Number(o.total||0)}
 const prodRows=Object.values(byProduct).sort((a,b)=>b.orders-a.orders).slice(0,10).map(x=>`<tr><td>${esc(x.name)}</td><td>${x.orders}</td><td>${x.revenue.toFixed(2)}</td></tr>`).join("");
 const byStatus={new:0,confirmed:0,preparing:0,shipped:0,delivered:0,cancelled:0};
 orders.forEach(o=>{if(byStatus[o.status]!=null)byStatus[o.status]++});
 return htmlResponse(`<header class="top"><div class="wrap"><div class="brand">Syria Commerce</div><div class="sub">التقارير والإحصائيات</div></div></header>
 <div class="layout"><nav><a href="/">🏠 الرئيسية</a><a href="/dashboard">👥 المسوقون</a><a href="/products">📦 المنتجات</a><a href="/orders">🧾 الطلبات</a><a href="/commissions">💰 العمولات</a><a href="/customers">👤 العملاء</a><a href="/reports">📊 التقارير</a><a href="/settings">⚙️ الإعدادات</a></nav>
 <main><div class="card section"><span class="badge">المرحلة 8</span><h1>التقارير والإحصائيات</h1><p class="muted">ملخص الأداء اعتماداً على الطلبات والمنتجات والعملاء الحاليين.</p></div>
 <div class="grid">
 ...
 <div class="card stat"><div class="muted">طلبات ملغاة</div><div class="num">${cancelled.length}</div></div>
 <div class="card stat"><div class="muted">المبيعات المسلّمة</div><div class="num">${revenue.toFixed(2)}</div></div>
 <div class="card stat"><div class="muted">العمولات</div><div class="num">${commissions.toFixed(2)}</div></div>
 <div class="card stat"><div class="muted">متوسط الطلب</div><div class="num">${avg.toFixed(2)}</div></div>
 <div class="card stat"><div class="muted">المنتجات</div><div class="num">${products.length}</div></div>
 <div class="card stat"><div class="muted">العملاء</div><div class="num">${customers.length}</div></div>
 </div>
 <div class="card section"><h2>حالات الطلبات</h2><div class="statusgrid">
 <div>جديد <b>${byStatus.new}</b></div><div>مؤكد <b>${byStatus.confirmed}</b></div><div>تجهيز <b>${byStatus.preparing}</b></div><div>شحن <b>${byStatus.shipped}</b></div><div>تسليم <b>${byStatus.delivered}</b></div><div>ملغي <b>${byStatus.cancelled}</b></div>
 </div></div>
 <div class="card section"><h2>أكثر المنتجات طلباً</h2><div style="overflow:auto"><table><thead><tr><th>المنتج</th><th>عدد الطلبات</th><th>القيمة</th></tr></thead><tbody>${prodRows||"<tr><td colspan=3>لا توجد بيانات بعد</td></tr>"}</tbody></table></div></div>
 <div class="card section"><h2>ملاحظة</h2><p class="muted">هذه التقارير حالياً مبنية على بيانات الذاكرة التجريبية. بعد ربط D1 ستتحول إلى بيانات حقيقية ودائمة.</p></div>
 </main></div>
 <style>
 *{box-sizing:border-box}body{margin:0;background:#f5f7fb;color:#172033;font-family:Arial,sans-serif}.top{background:#111827;color:#fff;padding:18px 22px}.wrap{max-width:1100px;margin:auto}.brand{font-size:24px;font-weight:700}.sub{opacity:.75;margin-top:5px}.layout{display:grid;grid-template-columns:220px 1fr;gap:18px;max-width:1100px;margin:22px auto;padding:0 16px}nav,.card{background:#fff;border:1px solid #e5e7eb;border-radius:16px;box-shadow:0 4px 18px #00000008}nav{padding:10px;height:max-content}nav a{display:block;padding:13px;border-radius:10px;color:#172033;text-decoration:none}nav a:hover{background:#f1f5f9}.section,.stat{padding:20px}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:14px;margin:14px 0}.num{font-size:27px;font-weight:700;margin-top:8px}.muted{color:#667085}.badge{display:inline-block;padding:6px 10px;border-radius:999px;background:#eef2ff}.statusgrid{display:grid;grid-template-columns:repeat(3,1fr);gap:10px}.statusgrid div{padding:14px;background:#f8fafc;border-radius:10px}.statusgrid b{float:left}table{width:100%;border-collapse:collapse}th,td{padding:12px;border-bottom:1px solid #eee;text-align:right}@media(max-width:700px){.layout{grid-template-columns:1fr}nav{display:grid;grid-template-columns:1fr 1fr}.statusgrid{grid-template-columns:1fr 1fr}}
 </style>`,"التقارير");
}
